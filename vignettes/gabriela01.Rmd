...
# ALL FORCES OF NATURE FORCES INCLUDED IN THE "SIM" SYSTEM 
all_data <- data_9 %>%
...
# HYDRO-GEOLOGICAL AND EXTREME TEMPERATURES DISASTERS' FILTERED SIM SYSTEM 
hydrogeo_tb <- data_9 %>%
    dplyr::bind_rows(data_10) %>%
    dplyr::mutate(death_year = lubridate::year(death_date),
                  computed_age = difftime(death_date, birth_date, 
...
# JUST FOR 2011 TRAGEDY
serrana_tb <- hydrogeo_tb %>%
    dplyr::filter(death_date > as.Date("2011-01-10"),
                  death_date < as.Date("2011-01-13"))


# WITHOUT 2011 TRAGEDY
hydrogeo_no_serrana_tb <- hydrogeo_tb %>%
...
all_data %>%
    ...
    ggplot2::ggtitle("Cumulative sum of deaths by forces of nature causes") +
...
```{r plot_causes_cumsum1, echo=FALSE, fig.width = 7, fig.height = 5}

# BY CAUSE: HYDRO-GEO AND EXT TEMP CAUSES
hydrogeo_tb %>%
    dplyr::group_by(cause, death_year) %>%
    dplyr::summarize(fatalities = dplyr::n()) %>%
    dplyr::mutate(cumsum = cumsum(fatalities)) %>%
    ...
    ggplot2::xlab("Year") +
    ggplot2::ylab("Cumulative number of fatalities") +
    ggplot2::ggtitle("Cumulative sum of deaths by hydro-geo and extreme temperature causes") +
    ggplot2::scale_color_brewer(palette = "Paired")
...
hydrogeo_tb %>%
    ...
    ggplot2::xlab("Year") +
    ggplot2::ylab("Cumulative number of fatalities") +
    ggplot2::ggtitle("Cumulative number of deaths")


```


```{r plot_cumsum2, echo=FALSE, fig.width = 7, fig.height = 5}

# WITHOUT 2011 SERRANA' CATASTROPHE
# CUMULATIVE DEATHS BY HYDRO-GEO AND EXT TEMP CAUSES
# TODO: REVIEW TITTLE
hydrogeo_no_serrana_tb %>%
    dplyr::group_by(death_year) %>%
    dplyr::summarize(fatalities = dplyr::n(), 
                     .groups = "drop") %>%
    dplyr::mutate(cumsum = cumsum(fatalities)) %>%
    ...
    ggplot2::xlab("Year") +
    ggplot2::ylab("Cumulative number of fatalities") +
    ggplot2::ggtitle("Cumulative number of (No Serrana Disaster)")
...
hydrogeo_tb %>%
    ...
    ggplot2::ggtitle("Total number of deaths by forces of nature by year")

```



```{r plot_total2, echo=FALSE, fig.width = 7, fig.height = 5}

# WITHOUT SERRANA DISASTER
# BY YEAR: HYDRO-GEO AND EXT TEMP CAUSES
# TODO: REVIEW TITTLE
hydrogeo_no_serrana_tb %>%
    dplyr::group_by(death_year) %>%
    ...
    ggplot2::xlab("Year") +
    ggplot2::ylab("Number of fatalities") +
    ggplot2::ggtitle("Total number of deaths by forces of nature by year w/o Serrana disaster")
...
hydrogeo_tb %>%
    ...
    ggplot2::xlab("Year") +
    ggplot2::ylab("Number of fatalities") +
    ggplot2::ggtitle("Death due to hydro-geological cause") + 
...
```{r plot_causes_sex_cumsum0, echo=FALSE, fig.width = 7, fig.height = 5}
...
all_data %>%
    ...
    ggplot2::ggtitle("Cumulative sum of deaths due to hydro-geological causes and sex") +
    ggplot2::scale_color_brewer(palette = "Dark2")
...
```{r plot_causes_sex_cumsum1, echo=FALSE, fig.width = 7, fig.height = 5}
...
```{r plot_causes_sex_cumsum2, echo=FALSE, fig.width = 7, fig.height = 5}
...
# BY SEX: EXTREME TEMPERATURES DISASTERS WITHOUT SERRANA TRAGEDY
hydrogeo_tb %>%
    dplyr::filter(death_date >= lubridate::as_date("1996-01-01")) %>% 
    dplyr::filter(sex %in% c("Male", "Female")) %>%
    dplyr::group_by(sex, cause, death_year) %>%
    ...
    ggplot2::xlab("Year") +
    ggplot2::ylab("Cumulative number of fatalities by sex") +
    ggplot2::ggtitle("Cumulative sum of deaths due to extreme temperatures causes and sex") +
    ggplot2::scale_color_brewer(palette = "Set1")
...
# NOTHING TO BE DONE. IS A GAP AND DONE
hydrogeo_tb %>%
    dplyr::filter(death_date >= lubridate::as_date("1996-01-01")) %>% 
