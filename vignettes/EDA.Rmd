...
This is the list of causes of deaths by natural forces.

```{r print_causes, echo=FALSE}

knitr::kable(causabas_tb)
...
plot_data %>%
  dplyr::group_by(Description, death_year) %>%
  dplyr::summarize(fatalities = dplyr::n(), 
                   .groups = "drop") %>%
  dplyr::arrange(Description, death_year) %>%
  ggplot2::ggplot(ggplot2::aes(x = death_year,
                               y = fatalities,
                               color = Description,
                               group = Description)) +
...
data_tb %>%
  dplyr::mutate(death_year = lubridate::year(date),
                Cause = stringr::str_sub(CAUSABAS, 1, 3)) %>%
  dplyr::left_join(causabas_tb, by = "Cause") %>%
  dplyr::filter(SEXO %in% 1:2) %>%
  dplyr::group_by(SEXO, Description, death_year) %>%
  dplyr::arrange(SEXO, Description, death_year) %>%
  dplyr::summarize(fatalities = dplyr::n()) %>%
  dplyr::mutate(cumsum = cumsum(fatalities)) %>%
  dplyr::ungroup() %>%
  ggplot2::ggplot(ggplot2::aes(x = death_year,
